    ...
    <AdminLayout>
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            onClick={openNew}
            className="w-full sm:w-auto min-h-11 px-5 py-2.5 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors"
          >
            New Post
          </button>
        </div>
          ...
          {paginatedPosts.map((post) => (
            <div key={post.id} className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => openEdit(post)} className="flex-1 min-h-10 px-3 py-2 border border-gray-200 rounded-lg text-sm text-teal-700 hover:bg-teal-50">Edit</button>
                <button onClick={() => handleDelete(post.id)} className="flex-1 min-h-10 px-3 py-2 border border-red-200 rounded-lg text-sm text-red-700 hover:bg-red-50">Delete</button>
              </div>
            </div>
          ))}
        </div>
        ...
        {editing && (
          <div className="fixed inset-0 bg-black/50 z-50 p-4 flex items-center justify-center">
            <div className="bg-white w-full max-w-3xl rounded-xl p-4 sm:p-6 space-y-4 h-[calc(100vh-2rem)] sm:h-auto max-h-[90vh] overflow-y-auto">
              ...
              <input className="w-full min-h-11 border rounded-lg px-3 py-2" placeholder="External URL (optional)" value={editing.externalUrl ?? ""} onChange={(e) => setEditing({ ...editing, externalUrl: e.target.value })} />
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <input className="flex-1 min-h-11 border rounded-lg px-3 py-2" value={tagInput} onChange={(e) => setTagInput(e.target.value)} placeholder="Add a tag" />
                  <button onClick={addTag} className="min-h-11 px-4 py-2 border rounded-lg">Add</button>
                </div>
